...
def doExportMail(output2, d):
    ...
    for stu in students:
        ...
        for cl in classes:
            allclasses = allclasses + "\t" + cl[2] + "\t" + cl[1] + "\t" + cl[0] + "\t" + cl[3] + "\t" + cl[4] + "\t" + \
                         cl[5]
...
def doExportAll(output2, d):
    ...
    with open(filename, 'wb') as f:
        f.write(
            "TA Name\tEmail\tAmount Scheduled\tYear\tDivision\tSkill Level\tSection\tClass Name(Short)\tClass Name (Long)\tClass Worth\tProfessor Name\tDay\tTime\tRoom\tOpen Seats\tSeats\n")
        for row in rows:
            wr = ""
            for r in row:
                wr = wr + str(r) + "\t"
