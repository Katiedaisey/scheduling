        ...
        if len(cl[3]) > 1:
            for c in cl[3]:
                try:
                    ...
                    block_in_Calendar(text=cl[2] + " (" + name + ")", open=cl[1], day=c, start=cl[4], end=cl[5],
            ...
            try:
                ...
                block_in_Calendar(text=cl[2] + " (" + name + ")", open=cl[1], day=cl[3], start=cl[4], end=cl[5],
                                  calendarFrame=calendarFrame)
            except:
                continue
            # info.append(cl[2] + ": " + name + " " + cl[4] + "-" + cl[5])
...
def doViewStudent(student):
    ...
    info = cur.fetchone()
    add_info_Calendar(text="Year: " + str(info[0]) + ", Div: " + str(info[1]) + ', Skill: ' + str(info[2]),
                      calendarFrame=calendarFrame)
    ...
    if sch[0] > 0:
        ...
        for cl in sch_classes:
            if len(cl[3]) > 1:
                for c in cl[3]:
                    try:
                        block_in_Calendar(text=cl[1] + " " + cl[2], open=1, day=c, start=cl[4], end=cl[5],
                                          calendarFrame=calendarFrame)
                    except:
                        continue
            else:
                try:
                    block_in_Calendar(text=cl[1] + " " + cl[2], open=1, day=cl[3], start=cl[4], end=cl[5],
                                      calendarFrame=calendarFrame)
                except:
                    continue
        ...
        for i in range(len(students)):
            if current_section != "any":
                ...
                student_tuples.append((int(globalvars.mat_prefs[stuID, colnum]),
                                       (students[i][1], students[i][0], students[i][2]),
...
def openaddselect():  # Schedule
    ...
    if scheduling == 'class':
        ...
        if current_section != "any":
            ...
            c = cur.execute('UPDATE Students SET Scheduled = ? WHERE StudentID = ?',
                            (float(value) + float(oldvalue), stu))
            c = cur.execute('UPDATE Sections SET Scheduled = ? WHERE SectionID = ?', (1, sec))
            c = cur.execute('UPDATE Sections SET StudentID = ? WHERE SectionID = ?',
