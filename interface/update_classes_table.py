def update_classes_table(filename):
    ...
    for entry in open(filename):
        ...
        if len(entry) > 11:
            ...
            div = entry[4]
            if 'A' in div:
                cur.execute('INSERT OR IGNORE INTO Sections_Division (SectionID, DivisionID) VALUES (?, ?)',
                            (SectionID, 1))
            elif 'B' in div:
                cur.execute('INSERT OR IGNORE INTO Sections_Division (SectionID, DivisionID) VALUES (?, ?)',
                            (SectionID, 2))
            ...
            cur.execute('''INSERT OR IGNORE INTO Sections_Times (SectionID, TimeID) VALUES (?, ?)''',
                        (SectionID, TimeID))
    ...
    for course in classes:
        ...
        for i in range(len(sections)):
            ...
            for j in range(len(othersections)):
                cur.execute('''INSERT OR IGNORE INTO Pref_Section_Section (SectionID1, SectionID2) VALUES (?, ?)''',
                            (sections[i][0], othersections[j][0]))
        conn.commit()
    ...
    for course in classes:
        ...
        for i in range(len(sections)):
            othersections = sections[:i] + sections[i + 1:]
            for j in range(len(othersections)):
                cur.execute('''INSERT OR IGNORE INTO Pref2_Section_Section (SectionID1, SectionID2) VALUES (?, ?)''',
                            (sections[i][0], othersections[j][0]))
        conn.commit()
