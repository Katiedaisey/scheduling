...
# update student tables
	...
	for entry in open(filename):
		...
		if skiprow > 0:
			...
			cur.execute('INSERT OR IGNORE INTO Students (Name, Email, ID, Year, 	Division, Skill) VALUES (?, ?, ?, ?, ?, ?)', (entry[2], entry[1], entry[3], year, div, sk))
			...
			if len(avails) > 0:
				...
				for avail in avails:
					...
					if data is None:
						...
						TimeID = data
					cur.execute('INSERT OR IGNORE INTO Pref_Student_Time (StudentID, TimeID) VALUES (?, ?)', (StudentID, TimeID))
			conn.commit()
			...
			if len(notavails) > 0:
				...
				for notavail in notavails:
					...
					if data is None:
						...
						TimeID = data
					cur.execute('INSERT OR IGNORE INTO Con_Student_Time (StudentID, TimeID) VALUES (?, ?)', (StudentID, TimeID))
			...
			if len(entry[12]) > 0:
				...
				for clas in classes:
					try:
						...
						cur.execute('''INSERT OR IGNORE INTO Pref_Student_Prof (StudentID, ProfessorID) VALUES (?,?)''', (StudentID, ProfID))
					except:
						continue
			...
			if len(entry[13]) > 0:
				...
				for clas in classes:
					try:
						cur.execute('SELECT ProfessorID FROM Professors WHERE Name = ?',  (clas,))
						ProfID = cur.fetchone()[0]
						cur.execute('''INSERT OR IGNORE INTO Con_Student_Prof (StudentID, ProfessorID) VALUES (?,?)''', (StudentID, ProfID))
					except:
						continue
